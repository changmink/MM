...
- [ ] **다운로드 흐름:**
  ...
  4. 임시 파일 크기가 `url_import_max_bytes`(§2.7) 초과 시 ffmpeg 프로세스 kill + 임시 파일 삭제 → `error: "too_large"`
  5. `url_import_timeout_seconds`(§2.7) 또는 요청 context 취소 시 ffmpeg 프로세스 kill → `error: "download_timeout"` (또는 context에 따라 `network_error`)
...
- [ ] **서버 검증:** PATCH 시 두 필드 모두 경계 검사. 범위 밖 값은 `400 {"error": "out_of_range", "field": "url_import_max_bytes" | "url_import_timeout_seconds"}` 반환. 저장 실패 시 `500 {"error": "write_failed"}`.
...
의존 방향: `cmd → handler → (importjob, urlfetch, convert, thumb, settings, media) → media`. `media`는 최하위, 상향 의존 금지.
