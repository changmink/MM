...
- [ ] FU3-I-1: SSE/JSON 헬퍼 3중복 통합 — `internal/handlerutil` 신설, `writeJSON`/`writeError`/`assertFlusher`/`writeSSEHeaders`/`writeSSEEvent`/`sseEmitter`를 `handler.go` + `handler/import/common.go` + `handler/convert/common.go` 세 곳에서 단일 출처로 이전.
- [ ] FU3-I-2-A: `*_compat_test.go` 셰임에 `Deprecated: transitional shim` 주석 추가 — 부모 패키지 테스트 이전 전까지 의도 가시화 (단기, 비차단).
...
- [ ] FU3-S-1: `internal/urlfetch/hls/download.go:232-242` materialize dispatch 루프를 labeled break로 정리 — `runImportURLWorkers` 패턴과 일관화.
- [ ] FU3-S-2: `importURLWorkers=2` / `hlsMaterializeParallelism=4` 상수 옆에 근거 주석.
- [ ] FU3-S-3: `runImportURLWorkers` 워커 진입부 이중 cancellation guard 의도 주석.
- [ ] FU3-S-4: `internal/urlfetch/hls/helpers_test.go`의 stub `NewClient`를 `newTestClient`로 개명 — production stub과 명명 분리.
- [ ] FU3-S-5: `convert/common.go sseEmitter` mutex 주석 — OnStart(핸들러 goroutine) + progress writer 두 호출자 명시.
- [ ] FU3-S-6: `upload.go uploadPNGAutoConvert` close 에러 wrap 통일 — A.1 패턴(`fmt.Errorf("...: %w", ...)`)과 비대칭 해소.
