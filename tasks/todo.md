...
- [ ] J1: `internal/importjob` 모듈 — `Job`/`Registry`/`Status`/`Event`/`URLState`/`Summary` 타입 + `Subscribe`/`Publish`/`Cancel`/`CancelURL`/`Remove`/`RemoveFinished`/`CancelAll` + 단위 테스트 10개 (Create ID, RejectsWhenFull, Subscribe broadcast, SlowConsumer drop, Unsubscribe, Cancel propagates ctx, CancelURL targets only N, Remove rejects active, RemoveFinished leaves active, CancelAll affects active)
- [ ] J2: graceful shutdown — `cmd/server/main.go`에 `signal.NotifyContext` + serverCtx, `httpServer.Shutdown(5s)` + `handler.Close()`. `Handler.serverCtx` 필드 + `Register(mux, dataDir, webDir, store, ctx)` 시그니처. 모든 핸들러 테스트의 `Register` 콜 사이트 일괄 갱신. 수동 SIGINT 검증.
- [ ] J3: `handleImportURL` registry 통합 — `Handler.registry`, `registry.Create(rel, urls)` (429 on full), worker goroutine + handler subscriber 패턴 (events 채널 drain → SSE write), `register` 이벤트 첫 emit, per-URL ctx `RegisterURLCancel`/`Unregister` (J5에서 활용), 잡 status 전이 (queued → running → completed/failed/cancelled). 회귀 + 신규 테스트 3개 (Register first event, HandlerDisconnect job continues, TooManyJobs 429).
- [ ] J4: `GET /api/import-url/jobs` + `GET /api/import-url/jobs/{id}/events` — `internal/handler/import_url_jobs.go` 신규, `handleListJobs` + `handleSubscribeJob` (snapshot replay + live), 라우터 분기 (`/jobs` 정확 매치 + `/jobs/` prefix). 핸들러 테스트 5개. 클라이언트 `bootstrapURLJobs()` + `subscribeToJob(jobId)` (EventSource), `register`/`snapshot` phase handler, app.js?v=22. 수동 시나리오 3개.
- [ ] J5: cancel + dismiss — `POST /api/import-url/jobs/{id}/cancel?index=N`, `DELETE /api/import-url/jobs/{id}`, `DELETE /api/import-url/jobs?status=finished`. worker가 per-URL ctx done 감지 → cancelled emit. 잡 status 결정 규칙 (succeeded≥1 → completed). 핸들러 테스트 6개. 클라이언트 row별 ✕ + 배치 헤더 "전체 취소" / "닫기" + footer "모두 지우기" + `removed` phase handler + `cancelled` 라벨, app.js?v=23. 수동 5개.
- [ ] J6: E2E 8개 시나리오 + `SPEC.md §2.6`/`§5.1` 본문 갱신 (잡 lifecycle 분리, 신규 엔드포인트 4종, SSE 신규 phase 3종, MaxQueuedJobs=100). `spec-url-import-persistence.md` 상단 `Status: merged` 노트.
