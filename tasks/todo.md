...
GIF + 짧은 동영상(≤50 MiB && ≤30s)을 animated WebP로 영구 변환. ffmpeg `libwebp_anim` SSE. 움짤 게이트 서버 재검증 + audio 자동 drop + warning. UI는 움짤 탭 한정 일괄 버튼 + 카드별 버튼. **자동 업로드 변환 없음** (수동만 — PNG→JPG와 다른 정책). settings 의존 없음(고정 파라미터).

- [x] CW0: SPEC §2.9 신설 + §3/§4/§5/§5.1/§8/§9 갱신 + 인코더 표기 정정(`libwebp_anim` 우선/`libwebp` 폴백) + tasks/plan.md Phase 29 + tasks/todo.md Phase 29 entry 작성. 구현 없음(선행 커밋).
- [ ] CW1: `internal/convert/webp.go` 신규 — `EncodeWebP(ctx, src, dstDir, baseName, cb)` (ffmpeg `libwebp_anim` 인코딩, atomic temp `.webpconvert-*.webp` → rename) + `ProbeStreamInfo(srcPath) (durationSec, hasAudio, err)` 헬퍼 + `webp_test.go` 12 케이스(정상 mp4/GIF/audio drop/fps 보존/손상/cancel/ffmpeg missing/atomic cleanup + ProbeStreamInfo 3개). RemuxTSToMP4의 `Callbacks`/`Result`/`watchTmp`/throttle 상수 재사용.
- [ ] CW2: `internal/handler/convert_webp.go` 신규 + `Handler.webpLocks sync.Map` 필드 + `Register`에 `requireSameOrigin(h.handleConvertWebP)` 라우트. POST `/api/convert-webp` SSE — 게이트 재검증(GIF 무조건 / 동영상은 50MiB+30s, duration은 thumb 캐시→backfill / 그 외 unsupported_input), 목표 `.webp` 충돌 거부, per-path lock, 5분 timeout, audio_dropped warning, delete_original 시 원본+사이드카 삭제(.jpg+.dur). `convert_webp_test.go` 16 케이스. **체크포인트 ②**: curl 5단계 검증.
- [ ] CW3: frontend — `web/convertWebp.js` 신규 모듈(SSE 클라이언트, TS→MP4 `convert.js` 패턴 미러), `index.html` `#convert-webp-modal` + `#convert-webp-all-btn` + main.js v=38→v=39, `style.css` `.webp-convert-btn`+`.convert-webp-all-btn`(민트 계열) + 모달, `dom.js` 9개 ref + all 버튼 ref, `main.js` `wireConvertWebP({browse})` 주입, `browse.js` GIF 카드 / `isClip` 동영상 카드에 "WebP로 변환" 버튼 + 움짤 탭(`view.type === 'clip'`) 한정 일괄 버튼 + `visibleClipPaths`/`selectedVisibleClipPaths` 헬퍼. 한국어 에러 라벨 12종 + warning 라벨 2종.
- [ ] CW4: chromedp e2e — `web_clip_to_webp_e2e_test.go` 6 시나리오(움짤 탭 일괄 버튼 / 다른 탭 숨김 / selection 라벨 / 비-움짤 제외 / 카드 버튼 노출 / 게이트 미충족 카드 미노출). `docker compose up -d --build` 후 11개 수동 검증 시나리오(일괄 변환·audio_dropped·GIF/짧은 mp4/긴 mp4/큰 mp4/PNG·delete_original·충돌·모바일·회귀 스모크).
