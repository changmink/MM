...
- [ ] PJ1: `internal/imageconv` 패키지 — `ConvertPNGToJPG(srcPath, destPath, quality)` + 흰 배경 합성(`draw.Draw` over) + atomic write(`os.CreateTemp` → `jpeg.Encode` → `os.Rename`) + quality 0..100 검증. 8 단위 테스트(RGB / RGBA 합성 / 손상 PNG / src 없음 / dest 디렉토리 없음 / quality 검증 / atomic glob / 확장자 호출자 결정).
- [ ] PJ2: `settings.AutoConvertPNGToJPG bool` 필드 추가 (기본 true) + `New()`에서 디스크 키 누락 시 true 강제(`map[string]json.RawMessage` 1차 디코드) + settings 모달 체크박스("PNG 업로드 시 JPG로 자동 변환"). `settings_test.go` Default/RoundTrip 갱신 + `TestNew_LegacyMissingKey` 신규. `handler/settings_test.go` boolean 타입 mismatch 케이스 추가. `web/settings.js` GET/PATCH 양쪽 처리. main.js v bump.
- [ ] PJ3: `handleUpload` PNG 자동 변환 분기 — `os.CreateTemp(.pngconvert-*.png)` → `imageconv.ConvertPNGToJPG` → `renameToUniqueDest`(신규 헬퍼, `O_CREATE|O_EXCL` 패턴) → `_1.jpg` 자동 suffix 시 `warnings:["renamed"]`. 변환 실패 시 원본 PNG로 폴백 + `warnings:["convert_failed"]` (응답 코드 201 유지). 응답에 `converted: bool` + `warnings: []` 추가. `files_test.go` 7 신규 케이스(success/RGBA/OFF/폴백/충돌/비-PNG/대문자.PNG). `web/fileOps.js` 응답에 한국어 inline 메시지 표시.
- [ ] PJ4: `POST /api/convert-image` 동기 핸들러 — `internal/handler/convert_image.go` 신규 + 라우트 등록. SafePath/`.png` 화이트리스트/디렉토리 거부/충돌(`already_exists`, 자동 suffix 없음)/30초 timeout/`delete_original` 시 원본+`.thumb/{name}.png.jpg` 삭제(실패 시 `delete_original_failed` warning). 응답: `{succeeded, failed, results:[]}`. 12 통합 테스트. **체크포인트 ① — curl 5단계 단독 검증**.
- [ ] PJ5: frontend — `web/convertImage.js` 신규 모듈(콜백 주입 패턴) + `#convert-image-modal` 마크업 + `CONVERT_IMAGE_ERROR_LABELS` 한국어. `web/browse.js` `buildImageGrid`에서 PNG 카드에 "JPG로 변환" 버튼, `renderView`에서 visible PNG 개수 기반 `#convert-png-all-btn` 표시. 모달 "변환 후 원본 PNG 삭제" 체크박스. `loadBrowse()` 갱신. **체크포인트 ② — 수동 E2E 12 시나리오**.
