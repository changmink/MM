...
- [x] BD-5: `web/lightbox.js` 분리 — 7 함수(openLightboxImage/openLightboxVideo/closeLightbox/deleteCurrentLightboxItem/playAudio/loadPlaylistTrack/renderPlaylist) + wireLightbox(lb·audio·키보드 listener 묶음). 사이클 회피: deleteCurrentLightboxItem의 browse(currentPath, false) 호출은 wireLightbox({onAfterDelete}) 콜백으로 주입(plan §6 #4). lightbox.js 138줄(≤200 AC pass), browse.js 354→248. `TestLightboxDelete_E2E` 6 + 5 web e2e 회귀 모두 통과.
- [x] BD-6: cache-bust v=42→v=43(BD 진행 중 base 가 이미 v=42). browse.js 248줄(≤250 AC pass). 잔여 함수 8개(browse / renderView / applyView / renderBrowseSummary / renderBreadcrumb / renderFileList / handleClick / wireBrowse), wireBrowse 는 wireSelection / wireLightbox delegate + convertAllBtn listener. node --check web/*.js 6개 모두 OK, 6 web e2e 모두 통과.
- [x] BD-7: 수동 브라우저 검증 12 시나리오(spec §6.2) 통과 + spec Status `merged` 갱신.
