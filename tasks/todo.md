...
- [ ] BD-0: spec accepted + Phase 32 entry 작성. 구현 없음(선행 commit).
- [ ] BD-1: `web/clipPlayback.js` 분리 — clipIOInstance / attachClipHoverPlayback / isClip / isClipConvertable. browse.js · cardBuilders 호출 사이트 import 갱신. `web_clip_hover_playback_e2e_test.go` 통과.
- [ ] BD-2: `web/visiblePaths.js` 분리 — visibleXxxPaths · selectedVisibleXxxPaths · updateConvertXxxBtn (5+3 함수). `web_png_select_e2e_test.go` · `web_clip_to_webp_e2e_test.go` 통과.
- [ ] BD-3: `web/selection.js` 분리 — bindEntrySelection / setSelected / refreshSelectionUI / updateCardSelection / syncCardSelectionStates / syncSelectionWithVisible / renderSelectionControls + `wireBrowse`의 selection toolbar 부분 이전. `web_drag_select_e2e_test.go` · `web_png_select_e2e_test.go` 통과.
- [ ] BD-4: `web/cardBuilders.js` 분리 — sectionTitle / buildImageGrid / buildVideoGrid / buildTable. `web_sticky_e2e_test.go` 통과.
